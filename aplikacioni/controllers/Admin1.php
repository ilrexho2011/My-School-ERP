...
class Admin extends CI_Controller
{
    ...
    /*     * * funksioni default, redirekton ne faqen login nese asnje admin nuk eshte loguar ende ** */
    ...
    function exam_marks_sms($param1 = '' , $param2 = '')
    {
        ...
        if ($param1 == 'send_sms') {
            ...
            // terheq piket e nxenesve per provimin e perzgjedhur
            foreach ($students as $row) {
        ...
        foreach($attendance_of_students as $row) {
            ...
            if ($attendance_status == 2) {
                ...
                if ($active_sms_service != '' || $active_sms_service != 'disabled') {
                    ...
                    $message        = 'Femija juaj' . ', ' . $student_name . ', nuk eshte paraqitur sot ne shkolle. Kontaktoni ne 0696919362.';
                    $this->sms_model->send_sms($message,$receiver_phone);
                }
            }
        }
            ...
            foreach ($students as $row)
            {
                ...
                if ($attendance_status == 2) {
                    ...
                    if ($active_sms_service != '' || $active_sms_service != 'disabled') {
                        ...
                        $parent_id      = $this->db->get_where('student' , array('student_id' => $row['student_id']))->row()->parent_id;
                        $receiver_phone = $this->db->get_where('parent' , array('parent_id' => $parent_id))->row()->phone;
                        $message        = 'Femija juaj' . ', ' . $student_name . ', nuk eshte paraqitur sot ne shkolle. Kontaktoni ne 033224093.';
                        $this->sms_model->send_sms($message,$receiver_phone);
                    }
                }
    ...
    function secretary($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/secretary_image/' . $secretary_id . '.jpg');
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            $this->email_model->account_opening_email('secretary', $data['email']); //DERGOHET EMAIL
            redirect(base_url() . 'index.php?admin/secretary/', 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['name']        = $this->input->post('name');
            $data['birthday']    = $this->input->post('birthday');
            ...
            $this->db->where('secretary_id', $param2);
            $this->db->update('secretary', $data);
            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/secretary_image/' . $param2 . '.jpg');
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/secretary/', 'refresh');
        } else if ($param1 == 'personal_profile') {
            $page_data['personal_profile']   = true;
            $page_data['current_secretary_id'] = $param2;
        } else if ($param1 == 'edit') {
        ...
        $page_data['secretarys']   = $this->db->get('secretary')->result_array();
        $page_data['page_name']  = 'secretary';
        $page_data['page_title'] = get_phrase('manage_secretary');
        $this->load->view('backend/index', $page_data);
    }
    ...
    function custom_sms($param1 = '' , $param2 = '')
    {
        ...
        if ($param1 == 'send_sms') {
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('message_sent'));
            redirect(base_url() . 'index.php?admin/custom_sms' , 'refresh');
        }
